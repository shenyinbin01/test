...
def run_phase4b(workspace_dir, output_root, project="price_tag_life"):
    ...
    def _call_node(node_name, system_prompt_path, user_prompt_content, output_filename, max_tokens, task_name=None):
    ...
    user_prompt = (
        f"# Chapter 001 Preflight Context Generation\n\n"
        f"## Chapter Goal\n{outline['goal']}\n\n"
        f"## Must Include\n" + "\n".join(f"- {item}" for item in outline["must_include"]) + "\n\n"
        f"## Runtime Canon Events\n" +
        "\n".join(f"- {e.get('event', e) if isinstance(e, dict) else e}"
                  for e in runtime_canon.get("confirmed_events", [])) + "\n\n"
    ...
    user_prompt = (
        ...
        f"## Chapter Number: 1\n\n"
        f"## Final Text\n{humanized_text[:3000]}\n\n"
        f"## Runtime Canon (current)\n{yaml.dump(runtime_canon, allow_unicode=True)}\n\n"
        f"## Output\n生成纯YAML（不要用```yaml代码块包裹），包含: confirmed_events（第一章实际发生事件）, \n"
        f"character_updates, world_updates, foreshadowing, canon_sync. \n"
    ...
    # Copy humanized → final.md（如 humanized 是 JSON 则从 rewritten_excerpt 提取）
    ...
    # Extract commit events for final canon
    commit_events = []
    ...
    if commit_path.exists():
        try:
            ...
            commit = yaml.safe_load(commit_raw)
            commit_events = commit.get("confirmed_events", []) or commit.get("changes", {}).get("plot_events", [])
        except Exception:
            pass
