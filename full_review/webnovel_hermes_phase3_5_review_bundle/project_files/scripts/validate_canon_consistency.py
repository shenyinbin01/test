...
def validate(output_root, canon_file, mode="real"):
    ...
    check_files = [
        ...
        (real_run_dir / "phase3_real_summary.json", "phase3_real_summary.json", REQUIRED_ANCHORS),
    ]
    ...
    for file_path, label, anchors in check_files:
        ...
        if result["forbidden_hits"]:
            for hit in result["forbidden_hits"]:
                ...
                warnings.append(f"{label}: 发现禁止模式 '{hit['pattern']}' — '{', '.join(hit['matches'])}'")
    ...
    if all_hits:
        passed = False
        for hit in all_hits:
            warnings.append(f"{hit['file']}: 发现禁止词 '{hit['pattern']}' (匹配: {', '.join(hit['matches'])})")
