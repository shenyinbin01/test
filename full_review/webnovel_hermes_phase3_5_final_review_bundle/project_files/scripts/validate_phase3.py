...
def validate_real(output_root):
    ...
    # ── 阶段三点五：summary 与 canon_consistency 一致性检查 ──
    canon_report_path = log_dir / "canon_consistency_report.json"
    if canon_report_path.exists():
        try:
            canon_data = json.loads(canon_report_path.read_text())
            # 检查 summary 与 report 一致
            summary_file = run_dir / "phase3_real_summary.json"
            if summary_file.exists():
                try:
                    summary_data = json.loads(summary_file.read_text())
                    summary_cc = summary_data.get("canon_consistency", {})
                    if summary_cc.get("checked", False):
                        if summary_cc.get("passed") != canon_data.get("passed"):
                            errors.append(
                                f"summary.canon_consistency.passed={summary_cc.get('passed')} 与 "
                                f"canon_consistency_report.json passed={canon_data.get('passed')} 不一致"
                            )
                        if canon_data.get("passed") and summary_data.get("has_failure", True):
                            errors.append(
                                "canon consistency 已通过但 summary 中 has_failure=true，应改为 false"
                            )
                        if not canon_data.get("passed") and not summary_data.get("has_failure", True):
                            errors.append(
                                "canon consistency 未通过但 summary 中 has_failure=false"
                            )
                except Exception as e:
                    errors.append(f"读取 phase3_real_summary.json 一致性失败: {e}")
        except Exception:
            pass
