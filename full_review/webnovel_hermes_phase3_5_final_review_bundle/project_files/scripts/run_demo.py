...
CANON_CONSTRAINTS_PATH = DATA_ROOT / "workspace" / "novels" / "price_tag_life" / "canon_constraints.yaml"


def load_canon_constraints():
    """加载 canon_constraints.yaml，返回 dict 或空 dict（文件不存在时）"""
    if CANON_CONSTRAINTS_PATH.exists():
        import yaml
        return yaml.safe_load(CANON_CONSTRAINTS_PATH.read_text())
    return {}


def render_canon_text_block():
    """读取 CANON_CONSTRAINTS_PATH 内容，包裹在标记中返回"""
    if not CANON_CONSTRAINTS_PATH.exists():
        return ""
    content = CANON_CONSTRAINTS_PATH.read_text(encoding="utf-8")
    return (
        "【CANON_CONSTRAINTS_BEGIN】\n"
        f"{content}\n"
        "【CANON_CONSTRAINTS_END】"
    )


def render_chapter_beat_hard_rules():
    """返回章节节拍硬约束文本，包裹在标记中"""
    rules = [
        "【CHAPTER_BEAT_HARD_RULES_BEGIN】",
        "【CHAPTER_BEAT_HARD_RULES】",
        '1. 禁止使用\u201c生命倒计时\u201d。',
        '2. 禁止使用\u201c倒计时\u201d描述标签。',
        '3. 禁止把\u201c即将归零\u201d解释为寿命、生命剩余时间、死亡倒计时。',
        '4. \u201c即将归零\u201d只能表达综合人生价值/风险/选择代价的显影正在逼近危险临界点。',
        '5. 父亲仍然病重存活，不能写已死。',
        '6. 不允许新增天秤会、系统、组织、寿命交换、触碰改命。',
        '7. 如果需要描述父亲头顶数字变化，只能写\u201c综合数值趋近于零\u201d或\u201c价格标签正在归零\u201d，不能写\u201c生命倒计时\u201d。',
        "【CHAPTER_BEAT_HARD_RULES_END】",
    ]
    return "\n".join(rules)


def save_prompt_input(task_name, mode, user_payload, prompt_path):
    """保存阶段三真实调用的 input 快照"""
    canon = load_canon_constraints()
    snap_dir = output_root / "phase3_logs" / "prompt_inputs"
    # 使用调用者作用域的 output_root；这里用全局变量不理想但兼容现有结构
    # 实际调用时外部传入 output_root
    pass


# ── 阶段三步骤函数 ──────────────────────────────────────

def phase3_story_bible(output_root, mode="mock", steps_filter=None):
    """阶段三 story_bible 节点"""
    if steps_filter and "story_bible" not in steps_filter:
        return None
    task_name = "story_bible"
    project_dir = DATA_ROOT / "workspace" / "novels" / DEMO_PROJECT["project_name"]
    project_yaml = project_dir / "project.yaml"

    input_data = {}
    if project_yaml.exists():
        import yaml
        input_data = yaml.safe_load(project_yaml.read_text())

    # 加载 canon 约束
    canon = load_canon_constraints()

    # 保存 prompt_input 快照
    prompt_input_dir = ensure_dir(output_root / "phase3_logs" / "prompt_inputs")
    prompt_path = CODE_ROOT / "templates" / "prompts" / "story_bible.md"
    snap = {
        "task_name": task_name,
        "mode": mode,
    ...
    snap_path = prompt_input_dir / f"{task_name}_input.json"
    write_json(snap_path, snap)

    # 生成输入文件（包含 canon 约束）
    enriched_input = {
        "project": input_data,
        "_canon_constraints_for_system": canon,
    }
    input_path = output_root / "phase3_inputs" / f"{task_name}_input.json"
    ensure_dir(input_path.parent)
    write_json(input_path, enriched_input)

    # 输出路径
    suffix = "real" if mode == "real" else "mock"
    md_output = output_root / f"phase3_{suffix}_run" / f"{task_name}_{suffix}.md"
    json_output = output_root / f"phase3_{suffix}_run" / f"{task_name}_{suffix}.json"
    ensure_dir(md_output.parent)

    log_path = output_root / "phase3_logs" / "deepseek_calls.jsonl"

    call_script = CODE_ROOT / "scripts" / "call_deepseek.py"
    cmd = [
    ...
    # real 模式额外注入 canon_constraints 路径
    if mode == "real":
        cmd.extend(["--real", "--log", str(log_path)])
        # 通过 --input 传递 enriched input（含 canon）
        cmd.extend(["--input", str(input_path)])
        # 写入 canon 文本临时文件
        canon_text = render_canon_text_block()
        if canon_text:
            canon_tmp = Path("/tmp/canon_text_block.txt")
            canon_tmp.write_text(canon_text, encoding="utf-8")
            cmd.extend(["--canon-text", str(canon_tmp)])
        # 更新 prompt_inputs 快照，增加 canon 注入字段
        snap["canon_injected"] = bool(canon_text)
        if canon_text:
            snap["rendered_system_prompt_preview"] = canon_text[:500]
        # 回写更新后的快照
        write_json(snap_path, snap)
    else:
        cmd.append("--mock")
    ...
    step_result = {
        ...
        "output_file": str(md_output),
    }

    # 写入 JSON
    write_json(json_output, step_result)
    return step_result


def phase3_chapter_beat(output_root, mode="mock", steps_filter=None):
    """阶段三 chapter_beat 节点"""
    if steps_filter and "chapter_beat" not in steps_filter:
        return None
    task_name = "chapter_beat"
    project_dir = DATA_ROOT / "workspace" / "novels" / DEMO_PROJECT["project_name"]
    beat_input = {
        "chapter": 1,
        "title": DEMO_CHAPTER_001.get("title", ""),
        "conflict": DEMO_CHAPTER_001.get("conflict", ""),
        "hook": DEMO_CHAPTER_001.get("hook", ""),
        "scenes": [
            {"order": s["order"], "title": s["title"], "location": s["location"],
             "emotion_target": s["emotion_target"], "key_lines": s.get("key_lines", [])}
            for s in DEMO_CHAPTER_001.get("scenes", [])
        ],
    }
    ...
    # 保存 prompt_input 快照
    prompt_input_dir = ensure_dir(output_root / "phase3_logs" / "prompt_inputs")
    prompt_path = CODE_ROOT / "templates" / "prompts" / "chapter_beat.md"
    snap = {
        "task_name": task_name,
        "mode": mode,
        "canon_constraints": canon,
        "user_payload": beat_input,
        "prompt_file": str(prompt_path),
        "created_at": datetime.now().isoformat(),
    }
    snap_path = prompt_input_dir / f"{task_name}_input.json"
    ...
    if mode == "real":
        ...
        if canon_text:
            ...
            cmd.extend(["--canon-text", str(canon_tmp)])
        # 写入 hard rules 临时文件
        hard_rules_text = render_chapter_beat_hard_rules()
        if hard_rules_text:
            hr_tmp = Path("/tmp/chapter_beat_hard_rules.txt")
            hr_tmp.write_text(hard_rules_text, encoding="utf-8")
            cmd.extend(["--hard-rules", str(hr_tmp)])
        # 更新 prompt_inputs 快照，增加 canon/hard-rules 注入字段
        snap["canon_injected"] = bool(canon_text)
        snap["hard_rules_injected"] = bool(hard_rules_text)
        if canon_text:
            snap["rendered_system_prompt_preview"] = canon_text[:500]
        if hard_rules_text:
            snap["rendered_user_payload_preview"] = hard_rules_text[:500]
        # 回写更新后的快照
        write_json(snap_path, snap)
    else:
        cmd.append("--mock")
...
def phase3_humanize(output_root, mode="mock", steps_filter=None):
    """阶段三 humanize 节点"""
    if steps_filter and "humanize" not in steps_filter:
        return None
    task_name = "humanize"
    project_dir = DATA_ROOT / "workspace" / "novels" / DEMO_PROJECT["project_name"]

    # 使用阶段二 mock draft 片段作为 humanize 输入
    draft_path = project_dir / "manuscript" / "drafts" / "chapter_001_draft.md"
    draft_content = ""
    if draft_path.exists():
        draft_content = draft_path.read_text(encoding="utf-8")[:2000]
    if not draft_content:
        draft_content = DEMO_POLISHED_EXCERPT

    humanize_input = {
        "draft_excerpt": draft_content,
        "target_style": "快节奏、强钩子、口语化、去AI腔",
        "focus_areas": [
            "替换'他感到''他意识到'为具体动作",
            "删除解释性旁白，用对话推动",
            "强化老人和客户对话口吻差异",
    ...
    # 保存 prompt_input 快照
    prompt_input_dir = ensure_dir(output_root / "phase3_logs" / "prompt_inputs")
    prompt_path = CODE_ROOT / "templates" / "prompts" / "humanize.md"
    snap = {
        "task_name": task_name,
        "mode": mode,
        "canon_constraints": canon,
        "user_payload": humanize_input,
        "prompt_file": str(prompt_path),
        "created_at": datetime.now().isoformat(),
    }
    snap_path = prompt_input_dir / f"{task_name}_input.json"
...
def run_phase3(output_root, mode="mock", steps_filter=None):
    """运行阶段三全部节点"""
    print(f"  [phase3] mode={mode}, steps={steps_filter or 'all'}")
    results = []

    nodes = [
        ("Story Bible", phase3_story_bible),
        ("Chapter Beat", phase3_chapter_beat),
        ("Humanize", phase3_humanize),
    ]

    has_failure = False
    for label, fn in nodes:
        print(f"  [{label}] ... ", end="", flush=True)
        try:
            r = fn(output_root, mode=mode, steps_filter=steps_filter)
            if r is None:
                print("⏭️ 跳过（不在 steps 中）")
                continue
            if r.get("success"):
                print(f"✅ {r.get('stdout', '')[:80]}")
            else:
                has_failure = True
                print(f"❌ {r.get('stderr', '')[:80] or r.get('stdout', '')[:80]}")
            results.append(r)
        except Exception as e:
            has_failure = True
            print(f"❌ {e}")
            results.append({"task_name": label.lower().replace(" ", "_"), "success": False, "error": str(e)})
...
def main():
    import argparse
    parser = argparse.ArgumentParser(description="run_demo — webnovel-hermes-wps Demo")
    parser.add_argument("--mock", action="store_true", help="mock 模式（默认）")
    parser.add_argument("--real", action="store_true", help="真实调用 DeepSeek（需同时 --phase3）")
    parser.add_argument("--phase3", action="store_true", help="启用阶段三流程")
    parser.add_argument("--steps", help="逗号分隔：story_bible,chapter_beat,humanize")
    ...
    if args.phase3:
        ...
        print(f"  [phase3] 节点: {steps_filter or 'story_bible,chapter_beat,humanize'}")
        print(f"  [phase3] 模式: {mode}")
        print()

        results, has_failure = run_phase3(output_root, mode=mode, steps_filter=steps_filter)
        ...
        # 如果是 real 模式，尝试运行 canon consistency 检查
        if mode == "real":
            canon_script = CODE_ROOT / "scripts" / "validate_canon_consistency.py"
            if canon_script.exists():
                print()
                print("  [canon consistency] ... ", end="", flush=True)
                import subprocess
                try:
                    cs_result = subprocess.run(
                        ["python", str(canon_script), "--mode", mode, "--output-root", str(output_root)],
                        capture_output=True, text=True, timeout=120,
                    )
                    canon_passed = cs_result.returncode == 0
                    # 读取 canon_consistency_report.json
                    report_path = output_root / "phase3_logs" / "canon_consistency_report.json"
                    if report_path.exists():
                        report = json.loads(report_path.read_text())
                        summary["canon_consistency"] = {
                            "checked": True,
                        ...
                        summary["canon_consistency"] = {
                            "checked": True,
                            "passed": canon_passed,
                            "errors": [f"validate_canon_consistency.py exit {cs_result.returncode}"],
                            "warnings": [],
                        }
                    if canon_passed:
                        print("✅")
                        # canon 通过时，覆盖 has_failure，恢复节点状态
                        has_failure = False
                        for r in results:
                            if r.get("success") is False and r.get("canon_error"):
                                r["success"] = True
                                r.pop("canon_error", None)
                    else:
                        print(f"❌ exit {cs_result.returncode}")
                        has_failure = True

                    # ── Retry 机制：chapter_beat 禁止词检测 ──
                    if mode == "real" and not canon_passed:
                        # 从 report 中提取 chapter_beat 的 forbidden_hits
                        chapter_beat_report = report.get("results", {}).get("chapter_beat", {})
                        forbidden_hits = chapter_beat_report.get("forbidden_hits", [])
                        forbidden_words = ["生命倒计时", "倒计时"]
                        if any(fw in str(forbidden_hits) for fw in forbidden_words):
                            print()
                            print("  [retry] chapter_beat 检测到禁止词, 将重试...")
                            # 构造 correction prompt
                            canon_text_block = render_canon_text_block()
                            hard_rules_text = render_chapter_beat_hard_rules()
                            # 读取上一次输出作为 base
                            chapter_beat_result = None
                            for r in results:
                                if r.get("task_name") == "chapter_beat":
                                    chapter_beat_result = r
                                    break
                            last_output = ""
                            if chapter_beat_result and chapter_beat_result.get("output_file"):
                                last_out_path = Path(chapter_beat_result["output_file"])
                                if last_out_path.exists():
                                    last_output = last_out_path.read_text(encoding="utf-8")
                            correction_prompt = (
                                "# 【Canon 修正重试】\n\n"
                                "## 上次输出（包含违规内容）\n\n"
                                f"{last_output}\n\n"
                                "## 检测到的违规项\n\n"
                                f"forbidden_hits: {json.dumps(forbidden_hits, ensure_ascii=False)}\n\n"
                                "## 完整 Canon 约束（必须遵守）\n\n"
                                f"{canon_text_block}\n\n"
                                "## 章节节拍硬约束（必须遵守）\n\n"
                                f"{hard_rules_text}\n\n"
                                "## 修正要求\n\n"
                                "请根据以上 canon 约束和 hard rules，重新生成 chapter_beat 内容。\n"
                                "严禁使用「生命倒计时」「倒计时」等违规表述。\n"
                                "请确保所有内容与 canon 约束完全一致。"
                            )
                            correction_path = output_root / "phase3_retry_inputs" / "chapter_beat_correction.md"
                            ensure_dir(correction_path.parent)
                            correction_path.write_text(correction_prompt, encoding="utf-8")
                            # 重新调用 chapter_beat，传入 correction prompt 作为 user input
                            retry_suffix = "real"
                            retry_md = output_root / f"phase3_{retry_suffix}_run" / f"chapter_beat_{retry_suffix}_retry1.md"
                            retry_json = output_root / f"phase3_{retry_suffix}_run" / f"chapter_beat_{retry_suffix}_retry1.json"
                            ensure_dir(retry_md.parent)
                            call_script = CODE_ROOT / "scripts" / "call_deepseek.py"
                            prompt_path = CODE_ROOT / "templates" / "prompts" / "chapter_beat.md"
                            log_path = output_root / "phase3_logs" / "deepseek_calls.jsonl"
                            retry_cmd = [
                                "python", str(call_script),
                                "--task-name", "chapter_beat_retry1",
                                "--system", str(prompt_path),
                                "--input", str(correction_path),
                                "--output", str(retry_md),
                                "--real", "--log", str(log_path),
                            ]
                            # 也加入 canon-text / hard-rules
                            if canon_text_block:
                                canon_tmp = Path("/tmp/canon_text_block.txt")
                                canon_tmp.write_text(canon_text_block, encoding="utf-8")
                                retry_cmd.extend(["--canon-text", str(canon_tmp)])
                            if hard_rules_text:
                                hr_tmp = Path("/tmp/chapter_beat_hard_rules.txt")
                                hr_tmp.write_text(hard_rules_text, encoding="utf-8")
                                retry_cmd.extend(["--hard-rules", str(hr_tmp)])
                            try:
                                ...
                                retry_step_result = {
                                    ...
                                    "output_file": str(retry_md),
                                    "retried": True,
                                }
                                write_json(retry_json, retry_step_result)
                                if retry_success:
                                    print("  [retry] ✅ 重试成功，替换原 chapter_beat 输出")
                                    # 替换 results 中的对应项
                                    for i, r in enumerate(results):
                                        if r.get("task_name") == "chapter_beat":
                                            results[i] = retry_step_result
                                            break
                                    # 将 retry 输出复制到原输出路径
                                    orig_md = output_root / f"phase3_{retry_suffix}_run" / f"chapter_beat_{retry_suffix}.md"
                                    orig_json = output_root / f"phase3_{retry_suffix}_run" / f"chapter_beat_{retry_suffix}.json"
                                    retry_md.write_bytes(orig_md.read_bytes() if orig_md.exists() else b"")
                                    retry_md.rename(orig_md)
                                    if retry_json.exists():
                                        retry_json.rename(orig_json)
                                    has_failure = False
                                else:
                                    print("  [retry] ❌ 重试仍失败")
                            except Exception as e:
                                print(f"  [retry] ❌ 重试异常: {e}")
                except Exception as e:
                    print(f"❌ {e}")
                    has_failure = True
                    summary["canon_consistency"] = {
        ...
        # 根据 canon consistency 更新 summary 状态
        cc = summary.get("canon_consistency", {})
        if cc.get("checked", False) and not cc.get("passed", False):
            has_failure = True
            # 更新每个节点的 errors
            for r in results:
                if r.get("success"):
                    r["success"] = False
                    r["canon_error"] = "canon consistency检查未通过"
            summary["has_failure"] = True
            summary["failed_count"] = summary["total_nodes"]
        else:
            # canon passed 或未检查时，使用实际节点状态
            summary["has_failure"] = has_failure
            summary["success_count"] = sum(1 for r in results if r.get("success"))
            summary["failed_count"] = sum(1 for r in results if not r.get("success"))
        summary_md_path = output_root / f"phase3_{suffix}_run" / f"phase3_{suffix}_summary.json"
        write_json(summary_md_path, summary)
        ...
        for r in results:
            ...
            extra = f" → {r.get('output_file', '')}" if r.get("success") else f" → {r.get('stderr', '')[:100]}"
            md_lines.append(f"- {icon} {task}{extra}")
        md_lines.extend([
            "",
            f"**总节点：** {len(results)}",
        ...
        md_path = output_root / f"phase3_{suffix}_run" / f"phase3_{suffix}_summary.md"
        md_path.write_text("\n".join(md_lines), encoding="utf-8")

        print()
        print("=" * 55)
        if has_failure:
            print(f"  阶段三 {mode} 完成 ⚠️ — 存在失败节点")
            sys.exit(1)
        else:
            print(f"  阶段三 {mode} 完成 ✅ — 所有节点成功")
        print("=" * 55)
    else:
        # 阶段二原有流程
        run_phase2_main()
