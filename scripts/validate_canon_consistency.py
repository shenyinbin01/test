...
规则源统一使用 canon_rules_loader.py（优先 .story-system/canon_patterns.yaml，
降级到 templates/default_canon_patterns.yaml），不再维护内联 fallback 常量。
...
# 初始化规则 — 从 canon_rules_loader 加载
print("  [validate_canon_consistency] 加载 canon 规则源...")
_RULES = load_canon_patterns()
FORBIDDEN_PATTERNS = _RULES["forbidden_patterns"]
WIDE_PATTERNS = _RULES["wide_patterns"]
REQUIRED_ANCHORS = _RULES["required_anchors"]
NEGATION_PREFIXES = _RULES["negation_prefixes"]
CANON_RULES_SOURCE = _RULES["source"]
print(f"  [validate_canon_consistency] 规则源: {CANON_RULES_SOURCE}")
print()
...
def validate(output_root, canon_file, mode="real"):
    ...
    report = {
        ...
        "canon_rules_source": CANON_RULES_SOURCE,
