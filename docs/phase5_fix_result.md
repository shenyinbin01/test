...
### 关于 doc_meta.yaml GitHub raw 展示问题

由于 GitHub raw 对 doc_meta.yaml 的 yaml 格式展示仍存在单行问题（本地验证确认为 11 行多行 YAML，但 GitHub raw 渲染时可能因换行符处理差异显示为一行），**公开复审以 doc_meta.json 作为权威状态文件**。

doc_meta.json 是标准 JSON object，可确保 GitHub raw 展示无误。
