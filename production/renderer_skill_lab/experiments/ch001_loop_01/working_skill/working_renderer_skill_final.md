...
## 12. LOOP Round 1 Experimental Overlay

This section is temporary for `ch001_loop_01` and must not be treated as merged main Skill.

### 12.1 Prefer functional aliveness over extra reaction

When a capable character faces an absurd mismatch, show aliveness through useful reaction:

- confirming the abnormality,
- preserving or rejecting evidence,
- changing priority,
- testing the rule boundary,
- choosing the least wasteful next action.

Do not add extra gestures, side-character embarrassment, or clever remarks merely to prove the character is alive.

### 12.2 Use one or two resource proofs only

For a late or obsolete resource, choose the sharpest one or two proof objects. Show:

- why it once mattered,
- why it is mismatched now,
- how it can still affect the current plan.

Do not inventory every item. Do not turn resource contrast into a catalogue.

### 12.3 Keep old conflict light but not weightless

When an old conflict is reactivated, do not inflate the old opponent into a major villain. Also do not flatten the old conflict into a joke.

Show only enough past pressure to explain why the old matter once mattered, then return to the present mismatch and action problem.

### 12.4 Convert absurdity into executable next action

An absurd task should end with an executable action hook. The character should see one or more possible rule-recognizable anchors or workaround paths.

Avoid ending on a quip, a summary, or pure investigation with no action seed.

### 12.5 Reduce side-character explanation load

If a side character appears, give them one structural function: public pressure, useful misread, constraint, or trigger question.

Do not let the side character repeatedly ask questions that merely allow the protagonist or narrator to explain the premise.

---

## 13. LOOP Round 2 Convergence Overlay

This section narrows the experiment for Round 3.

### 13.1 Do not expand the solution space

In the final convergence pass, do not add new props, institutions, records, weapons, helpers, or procedures merely to sharpen the hook.

Use only:

- objects already present in the seed or current draft,
- setting elements already required by the seed,
- logical consequences of the task,
- minimal side-character support.

### 13.2 Resource as rule-perspective evidence

If a late resource proves the rule-carrier is still seeing an earlier version of reality, let one resource carry that proof.

The resource should alter the protagonist's plan. Do not add more examples after the proof has landed.

### 13.3 Avoid engineering vocabulary in prose

Do not use analytical terms such as anchor, data, rule boundary, mechanism, workflow, or proof layer inside the draft unless the world itself would use those words.

Translate analysis into story action: find the place, preserve the object, ask the question, stage the attempt.

### 13.4 Final hook should be concrete but light

The ending should name the next attempt clearly enough to pull the reader forward, but it should not fully solve the task.

The best final beat leaves:

- a target to find,
- a first method to try,
- an absurd fallback implied or lightly stated.

---

## 14. LOOP Final Consolidation

This final working version preserves the Round 3 convergence direction.

### 14.1 Final draft priority for this experimental branch

When the chapter seed contains a high-status character, a late low-level resource, an obsolete or impossible task, and an old conflict, prioritize:

1. one clear pressure image,
2. one concrete late-resource proof,
3. one brief old-conflict contrast,
4. one protagonist-led rule test,
5. one concrete absurd action hook.

Do not maximize all axes at once. A light, readable scene with a sharp next action is preferred over a fully proved explanation.

### 14.2 Final anti-regression locks

In the final draft, reject:

- inventorying all rewards,
- letting side characters explain the premise,
- adding new props to make the hook cleverer,
- turning old conflict into heavy revenge,
- turning the protagonist into a task processor,
- ending only on a joke without an action problem.
